"""Save input | GT | pred | overlay grids for N test images to artifacts/preds/.

A segmentation repo without pictures is uncheckable; this script is the bridge
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--num-samples", type=int, default=8, metavar="N",
                   help="How many test images to visualise (default: 8)")
    p.add_argument("--out-dir", type=str, default="artifacts/preds", metavar="DIR",
                   help="Output directory for PNGs (default: artifacts/preds)")
    p.add_argument("--data-dir", type=str, default=None, help="Override data dir from checkpoint config")
    return p.parse_args()


def _save_sample(
    image: torch.Tensor, gt: torch.Tensor, pred: torch.Tensor, path: Path
) -> None:
