"""Train U-Net on Oxford-IIIT Pet and save the best checkpoint."""
...
from unet_pet_seg.trainer import Trainer, TrainingLogger
...
def resolve_config(args: argparse.Namespace) -> Config:
    cfg_dict: dict[str, typing.Any] = {f.name: f.default for f in dataclasses.fields(Config)}
...
def main(cfg: Config, out_dir: str, resume: str | None = None) -> None:
    ...
    if is_main:
        logger: TrainingLogger = Logger(run_dir)
    else:
    ...
    if is_main:
        ...
        metrics_payload = {
            ...
            "per_class_iou": {
                name: round(score, 4) for name, score in zip(CLASS_NAMES, test_iou.tolist())
            },
        }
